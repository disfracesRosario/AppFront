    ...
    axios
      ...
      .then(response => {
        ...
        const generateRemitoPDF = async (responseData) => {
          ...
          page.drawText(`Telefono: ${responseData.clientPhone}`, { x: 100, y: height - 269, fontSize });
          page.drawText(`Tipo: ${responseData.type}`, { x: 100, y: height - 284, fontSize });
          ...
          page.drawText(`Desde ${data.reservationDate} Hasta ${data.deadline}`, { x: 290, y: height -360, fontSize });

        
          responseData.costumeIds.forEach((detail, index) => {
          
            page.drawText(`${detail}`, { x: 120, y: height - 360 - (20 * index), fontSize });
          });

          responseData.names.forEach((detail, index) => {
          
            page.drawText(`${detail}`, { x: 170, y: height - 360 - (20 * index), fontSize });
          });
